...
- **资源同步防漏更**：更新时默认强制重新下载前端资源与后端认证 / 站点资源，官方对同一版本号重新发布的内容也不会漏更
- **GitHub 加速友好**：支持 Token 与代理（默认自动跟随 Windows 系统代理，也可手动指定 http 代理；下载、git 均生效）
...
│   ├── tar\                   # 便携版 bsdtar（仅系统无内置 tar 时自动部署）
│   └── uv\                    # 包管理：替代pip (约束依赖版本)
...
> **强烈建议首次运行前先完成配置**：打开面板后先点「配置」，设置好 **代理**（默认「系统代理」自动跟随 Windows 系统代理，代理软件开系统代理即可；也可选「http」手动填写地址 / 端口）和 **GitHub Token**（`github_token`），再点启动服务。国内网络环境下不配代理，首次下载便携版组件、站点资源与克隆代码可能很慢甚至失败；填写规则见 [详细说明](README-advanced.md) 中「代理」一节。

1. 双击 `MoviePilot-V3.exe` 打开面板（默认显示主窗口，可在配置中改为启动即驻留托盘）
2. 点击 **启动服务**：首次运行会自动完成以下准备（只需一次）：
   - 下载便携版 **nginx**、**Git**、**Python 3.14.7**、**uv**（压缩包保存在 `tmp目录`，解压到 `runtime目录`；旧系统未内置 tar 时会额外自动部署便携版 bsdtar）
